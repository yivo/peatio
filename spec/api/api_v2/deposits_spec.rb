describe APIv2::Deposits, type: :request do
  ...
  describe 'GET /api/v2/deposits' do
    before do
      ...
      create(:deposit, member: member, currency: 'cad')
      create(:deposit, member: member, currency: 'cad', txid: 1, amount: 520)
      create(:deposit, member: member, currency: 'btc', created_at: 2.day.ago, txid: 'test', amount: 111)
      create(:deposit, member: other_member, currency: 'cad', txid: 10)
    end
    ...
    it 'deposits currency cad' do
      signed_get '/api/v2/deposits', params: { currency: 'cad' }, token: token
      result = JSON.parse(response.body)
      expect(result.size).to eq 2
      expect(result.all? { |d| d['currency'] == 'cad' }).to be_truthy
    end
